...
public class VarRecordPool implements RecordPool
{
	...
	{
		long realSize = 0;
		KeyIterator itr = this.accesser.iterator();
		while (itr.hasNext())
		{
	...
	@Override
	public synchronized KeyIterator iterator()
	{
	...
	private class SafeBlockIterator
	{
		...
		private LinkedBuffer curPreload = null;
		...
		private boolean hasNext() throws IOException
		{
			...
			preloadNext();
			return curPreload != null;
		}

		private void preloadNext() throws IOException
		{
			if (count >= orignPoolSize)
			{
				this.curPreload = null;
			}

			curPreload = tryGet(curKey);
		}
		...
		public LinkedBuffer next() throws IOException
		{
			...
			LinkedBuffer buffer = curPreload;
			if (null == buffer)
			{
